...
export default function ProductReviews({ product }) {
  const { addReview, getProductReviews, getProductReviewStats, isHydrated } = useStore();
  ...
  const formattedAverage = useMemo(() => (stats.count > 0 ? stats.average.toFixed(1) : "New"), [stats.average, stats.count]);
      ...
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-7 text-zinc-400">
            Reviews shown here are submitted by shoppers on this device. Be the first to help future customers choose the right product.
          </p>
        </div>
        <div className="rounded-[24px] border border-white/8 bg-white/[0.03] px-5 py-4">
          ...
          <div className="mt-2 flex items-center gap-2 text-white">
            ...
            <span className="text-2xl font-semibold">{formattedAverage}</span>
      ...
      <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <form onSubmit={handleSubmit} className="rounded-[28px] border border-white/8 bg-white/[0.03] p-6">
          ...
          <div className="mt-5 space-y-4">
            ...
            <ReviewInput label="Name" value={name} onChange={setName} placeholder="Your name" required />
        ...
        <div className="space-y-4">
          {!isHydrated ? (
            <ReviewEmptyState title="Loading reviews" text="Checking for customer reviews saved on this device." />
          ) : reviews.length === 0 ? (
